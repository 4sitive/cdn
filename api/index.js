...
exports.handler = async (event) => {
    ...
    console.log(JSON.stringify(event, function (key, value) { return key === 'body' ? undefined : value }))
    ...
    try {
        const exists = await S3.headObject({Bucket: process.env.AWS_S3_BUCKET, Key: key}).promise().then(() => true, err => {
